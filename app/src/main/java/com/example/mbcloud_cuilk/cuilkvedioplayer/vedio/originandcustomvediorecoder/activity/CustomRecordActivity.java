...
import com.example.mbcloud_cuilk.cuilkvedioplayer.vedio.originandcustomvediorecoder.util.VideoUtils;
        ...
        switch (view.getId()) {
            case R.id.record_control: {
                if (mRecorderState == STATE_INIT) {
                    ...
                    currentVideoFilePath = getSDPath(getApplicationContext()) + getVideoName();//=:  /storage/emulated/0/RecordVideo/VID_20180524_143340.mp4
                    //开始录制视频
                    if (!startRecord())
                        return;
